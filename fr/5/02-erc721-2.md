...
Mais — `ZombieOwnership` hérite déjà de `ZombieBattle` — comment pourrait-il aussi hériter de `ERC721` ?
...
2. Déclarez que `ZombieOwnership` hérite de `ZombieBattle` ET de `ERC721`
